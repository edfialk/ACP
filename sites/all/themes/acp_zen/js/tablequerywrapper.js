 ...
 * @constructor
 */
var TableQueryWrapper = function(query, container, options, filters) {
...
TableQueryWrapper.prototype.sendAndDraw = function() {
  jQuery('#numrows').html('<div class="progress"><div class="progress-bar progress-bar-striped active" style="width:100%;"></div></div>');
  this.query.abort();

  var queryClause = 'where A="Dataset Name"', queryClauses = [];

  if (Object.keys(this.filters.params).length > 0){
    var paramClauses = [];
    for (var key in this.filters.params){
      if (this.filters.params[key] === true){
        paramClauses.push(key + '!=""');
      }
    }
    queryClauses.push(paramClauses.join(' and '));
  }
  if (this.filters.spatial !== 'every'){
    queryClauses.push('J="'+this.filters.spatial+'"');
  }
  if (this.filters.temporal !== 'every'){
    queryClauses.push('I="'+this.filters.temporal+'"');
  }

  if (queryClauses.length > 0){
    queryClause += ' OR ' + queryClauses.join(' and ');
  }

  // queryClause += paramClause + ' ' + spatialClause;
  queryClause += this.sortQueryClause;
   // + ' ' + this.pageQueryClause;
...
TableQueryWrapper.prototype.handleResponse = function(response) {
  ...
  if (response.isError()) {
    ...
    jQuery('#numrows').html(response.getMessage());
  } else {
    this.currentDataTable = response.getDataTable();
    jQuery('#numrows').html('Displaying ' + this.currentDataTable.getNumberOfRows() + ' rows.');
...
TableQueryWrapper.prototype.setPageQueryClause = function(pageIndex) {
  ...
  // this.pageQueryClause = 'offset ' + newStartRow;
  // Note: row numbers are 1-based yet dataTable rows are 0-based.
  this.tableOptions.firstRowNumber = newStartRow + 1;
  return true;
};

TableQueryWrapper.prototype.setOptionQueryClause = function(option){
  var value = this.tableOptions[option];
